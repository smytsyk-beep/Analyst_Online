...
export default function OmniDashHero({ t, lang }: Props) {
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-white to-soft-bg pb-16 pt-16 md:pt-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        {/* Badge */}
        <div className="inline-flex items-center gap-2 rounded-md border border-omni-blue/20 bg-omni-blue/5 px-4 py-1.5 text-xs font-semibold text-omni-blue">
          <span className="h-1.5 w-1.5 rounded-full bg-omni-blue" />
          {t.heroBadge}
        </div>

        {/* Title */}
        <h1 className="mt-6 max-w-3xl text-3xl font-extrabold tracking-tight text-omni-navy sm:text-4xl md:text-[3.75rem] md:leading-[1.1]">
          {t.heroTitle}
        </h1>
        <p className="mt-5 max-w-2xl text-lg text-omni-navy/70">{t.heroSubtitle}</p>
        ...
        <div className="mt-10 flex flex-wrap gap-8">
          {t.heroStats.map((s, i) => (
            <div key={s.label} className="flex items-center gap-4">
              {i > 0 && <div className="h-8 w-px bg-grid-divider" />}
              <div>
                <div className="text-2xl font-bold text-omni-navy">{s.value}</div>
                <div className="mt-0.5 text-sm text-omni-navy/60">{s.label}</div>
              </div>
            </div>
          ))}
        </div>
